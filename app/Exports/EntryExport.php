...
class EntryExport implements FromCollection, WithMapping, WithHeadings, WithStyles, WithEvents
{
    ...
    public function map($entry): array
    {
        ...
        return [$entry->id, $entry->user->name, $entry->date, $entry->title, $entry->type, $entry->value, $entry->company, $entry->category->name, $tags, $entry->frequency, $files, $entry->info, date_format($entry->created_at, 'd-m-Y'), date_format($entry->updated_at, 'd-m-Y')];
    }
    ...
    {        
        return ['ID', 'USER', 'DATE', 'TITLE', 'TYPE', 'VALUE', 'COMPANY', 'CATEGORY', 'TAGS', 'FREQUENCY', 'FILES', 'INFO', 'CREATED', 'UPDATED'];
    }
