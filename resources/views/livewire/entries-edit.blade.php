    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <form wire:submit="save">
            ...
            <div class="mx-auto w-11/12">
                ...
                <!-- Type -->
                <div
                    class="flex flex-row justify-start items-center mt-6 py-2 px-2 rounded-md gap-3 {{ $type == 0 ? 'bg-green-100' : 'bg-red-100' }}  ">
                    <div>
                    ...
                    <div>
                        <label class="inline-flex cursor-pointer pt-2">
                            ...
                            value="{{ $entry->type }}" {{ $entry->type == 1 ? 'checked' : '' }} class="sr-only peer">
                            <div
                ...
                <div class="flex flex-row">
                    ...
                    <div wire:ignore class="w-full">
                        <select wire:model="selectedTags" name="selectedTags" id="selectedTags" multiple>
                            @foreach ($tags as $tag)
                                <option value="{{ $tag->id }}" 
                                @foreach ($this->selectedTags as $tagSelected)
                                    @if (old('selectedTags') == $tagSelected) selected @endif
                                @endforeach
                                >
                                    {{ $tag->name }}</option>
                            @endforeach
                ...
                <div class="relative">
                    <select wire:model.live="frequency" name="frequency" id="frequency"
                        ...
                        @foreach ($frequencies as $freq)
                            <option value="{{ $freq }}" class="text-green-600"
